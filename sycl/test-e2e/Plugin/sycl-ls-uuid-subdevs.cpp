 ...
 * devices. */
// REQUIRES:  run-mode, gpu, level_zero
...
// RUN: %if run-mode %{ env ONEAPI_DEVICE_SELECTOR="level_zero:*" env ZE_FLAT_DEVICE_HIERARCHY=COMPOSITE env ZEX_NUMBER_OF_CCS=0:4 sycl-ls --verbose >%t.default.out %}
// RUN: %if run-mode %{ FileCheck %s --input-file %t.default.out %}
